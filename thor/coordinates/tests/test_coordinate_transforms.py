...
X_AXIS_SPHERICAL = np.array([1.0, 0.0, 0.0])
NEG_X_AXIS_SPHERICAL = np.array([1.0, np.pi, 0.0])
Y_AXIS_SPHERICAL = np.array([1.0, np.pi / 2, 0])
NEG_Y_AXIS_SPHERICAL = np.array([1, 3 * np.pi / 2, 0.0])
Z_AXIS_SPHERICAL = np.array([1.0, 0.0, np.pi / 2])
NEG_Z_AXIS_SPHERICAL = np.array([1.0, np.pi, -np.pi / 2])
...
def __cardinalityCartesian(func):
    ...
    for s_axis, c_axis in zip(SPHERICAL_AXES, CARTESIAN_AXES):
        coords = func(
            s_axis[0:1], s_axis[1:2], s_axis[2:3], *[ZERO_V for i in range(3)]
        )
        np.testing.assert_array_almost_equal(
            np.concatenate([coords[0], coords[1], coords[2]]), c_axis
        )
    return
...
def __cardinalitySpherical(func):
    ...
    for s_axis, c_axis in zip(SPHERICAL_AXES, CARTESIAN_AXES):
        coords = func(
            c_axis[0:1], c_axis[1:2], c_axis[2:3], *[ZERO_V for i in range(3)]
        )
        np.testing.assert_array_almost_equal(
            np.concatenate([coords[0], coords[1], coords[2]]), s_axis
        )
    return
...
def test_transformCoordinates_representations():
    ...
    for frame in ["equatorial", "ecliptic"]:
        ...
        np.testing.assert_allclose(
            np.linalg.norm(coords_cartesian, axis=1), np.ones(100)
        )
        ...
        # Check that the radius is 1
        np.testing.assert_allclose(
            np.linalg.norm(coords_cartesian[:, :3], axis=1), np.ones(100)
        )
...
def test_transformCoordinates_frames():
    ...
    coords_hcrs_spherical = np.array(
        [coords_hcrs.distance.value, coords_hcrs.ra.value, coords_hcrs.dec.value]
    ).T
    coords_hcrs.representation_type = "cartesian"
    coords_hcrs_cartesian = np.array(
        [coords_hcrs.x.value, coords_hcrs.y.value, coords_hcrs.z.value]
    ).T
    ...
    coords_hcrs = SkyCoord(
        ra=lon * u.degree, dec=lat * u.degree, distance=rho * u.AU, frame="hcrs"
    )
    ...
    coords_eclipiau76_spherical = np.array(
        [
            coords_eclipiau76.distance.value,
            coords_eclipiau76.lon.value,
            coords_eclipiau76.lat.value,
        ]
    ).T
    coords_eclipiau76.representation_type = "cartesian"
    coords_eclipiau76_cartesian = np.array(
        [
            coords_eclipiau76.x.value,
            coords_eclipiau76.y.value,
            coords_eclipiau76.z.value,
        ]
    ).T
