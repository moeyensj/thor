...
def test_all_duplicates(simple_orbits, all_duplicates_orbit_members):
    ...
    assert len(unique_obs_ids) == len(
        filtered_members
    )  # Ensure no duplicates in 'obs_id'
...
def test_mixed_duplicates(simple_orbits, mixed_duplicates_orbit_members):
    ...
    assert pc.all(
        pc.equal(filtered_members.apply_mask(mask).orbit_id, pa.scalar("2"))
    ).as_py()
    ...
    assert pc.all(
        pc.equal(filtered_members.apply_mask(mask).orbit_id, pa.scalar("4"))
    ).as_py()
