...
__all__ = ["_handleUserConfig", "Configuration", "Config"]
...
DEFAULT_ODP_CONFIG = {
    ...
    "observations_chunk_size": 1000000,
    "num_jobs": NUM_JOBS,
    "backend": BACKEND,
    "backend_kwargs": BACKEND_KWARGS,
    "parallel_backend": PARALLEL_BACKEND,
    ...
    def __init__(
        ...
        if range_shift_config is None:
            ...
            self.RANGE_SHIFT_CONFIG = _handleUserConfig(
                range_shift_config, DEFAULT_RANGE_SHIFT_CONFIG
            )
        ...
        if cluster_link_config is None:
            ...
            self.CLUSTER_LINK_CONFIG = _handleUserConfig(
                cluster_link_config, DEFAULT_CLUSTER_LINK_CONFIG
            )

        if iod_config is None:
            self.IOD_CONFIG = DEFAULT_IOD_CONFIG
        else:
            self.IOD_CONFIG = _handleUserConfig(iod_config, DEFAULT_IOD_CONFIG)

        if od_config is None:
            self.OD_CONFIG = DEFAULT_OD_CONFIG
        else:
            self.OD_CONFIG = _handleUserConfig(od_config, DEFAULT_OD_CONFIG)

        if odp_config is None:
            self.ODP_CONFIG = DEFAULT_ODP_CONFIG
        else:
            self.ODP_CONFIG = _handleUserConfig(odp_config, DEFAULT_ODP_CONFIG)
        ...
        if contamination_percentage is not None:
            ...
            components = [self.IOD_CONFIG, self.OD_CONFIG, self.ODP_CONFIG]
            self.updateConfigs(
                "contamination_percentage", contamination_percentage, components
            )
