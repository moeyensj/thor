...
def _calc_gnomonic_rotation_matrix(coords_cartesian: jnp.ndarray) -> jnp.ndarray:
    ...
    v, vmag_set = lax.cond(
        ((r_dot_v > (1 - FLOAT_TOLERANCE)) & (r_dot_v < (1 + FLOAT_TOLERANCE)))
        | (v_mag < FLOAT_TOLERANCE),
    ...
    R1 = lax.cond(
        ...
        lambda vp, c: jnp.identity(3)
        + vp
        + jnp.linalg.matrix_power(vp, 2) * (1 / (1 + c)),
        vp,
        c,
    )
...
_cartesian_to_gnomonic_vmap = jit(
    vmap(_cartesian_to_gnomonic, in_axes=(0, None), out_axes=(0, None))
)
...
def cartesian_to_gnomonic(
    ...
    coords_gnomonic, M_matrix = _cartesian_to_gnomonic_vmap(
        coords_cartesian, center_cartesian
    )
