...
from ..orbits import generateEphemerisUniversal, propagateUniversal, shiftOrbitsOrigin
...
class MJOLNIR(Backend):
    ...
    def _propagateOrbits(self, orbits, t1):
        ...
        propagated = propagateUniversal(
            orbits_, t0_tdb, t1_tdb, mu=self.mu, max_iter=self.max_iter, tol=self.tol
        )
    ...
    def _generateEphemeris(self, orbits, observers):
        ...
        for observatory_code in observer_states["observatory_code"].unique():
            ...
            observer_selected = observer_states[
                observer_states["observatory_code"].isin([observatory_code])
            ]
            ...
            if set(velocity_cols).intersection(set(observer_selected.columns)) == set(
                velocity_cols
            ):
