...
def memory_snapshot(request, git_branch_or_revision):
    ...
    plt.ylim(
        bottom=min_mem_usage * 0.8, top=max_mem_usage + (max_mem_usage * 0.1)
    ...
    plt.savefig(
        f"{root_path}/{git_branch_or_revision}/{TEST_ORBIT_ID}-{request.node.name}.png"
    )
...
def memory_filtered_observations():
    ...
    return Observations.from_parquet(
        FIXTURES_DIR / f"{TEST_ORBIT_ID}/filtered_observations.parquet"
    )
...
def memory_clusters():
    ...
    cluster_members = ClusterMembers.from_parquet(
        FIXTURES_DIR / f"{TEST_ORBIT_ID}/cluster_members.parquet"
    )
...
def memory_od_orbits():
    ...
    od_orbits = FittedOrbits.from_parquet(
        FIXTURES_DIR / f"{TEST_ORBIT_ID}/od_orbits.parquet"
    )
...
def test_filter_observations(
    ...
    observations = filter_observations(
        memory_observations, memory_test_orbit, memory_config
    )
...
@pytest.mark.memory
@pytest.mark.parametrize("memory_config", CONFIG_PROCESSES, indirect=True)
def test_cluster_and_link(
    memory_transformed_detections, memory_config, ray_cluster, memory_snapshot
):
...
def test_link_test_orbit(
    memory_snapshot, memory_config, memory_observations, memory_test_orbit, ray_cluster
):
