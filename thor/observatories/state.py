...
from ..utils import _checkTime, readMPCObservatoryCodes, setupSPICE
...
def getObserverState(
    observatory_codes, observation_times, frame="ecliptic", origin="heliocenter"
):
    """
    ...
    if type(observatory_codes) not in [list, np.ndarray]:
        err = "observatory_codes should be a list or `~numpy.ndarray`."
        raise TypeError(err)
    ...
    for code in observatory_codes:
        if np.any(
            observatories[observatories.index == code][["longitude_deg", "cos", "sin"]]
            .isna()
            .values
            == True
        ):
        ...
        longitude = observatories[observatories.index == code]["longitude_deg"].values[
            0
        ]
        ...
        state = getPerturberState(
            "earth", observation_times, frame=frame, origin=origin
        )
        ...
        r_obs = np.array(
            [rg + rm @ o_vec_ITRF93 for rg, rm in zip(state[:, :3], rotation_matrices)]
        )
        ...
        v_obs = np.array(
            [
                ...
                for vg, rm in zip(state[:, 3:], rotation_matrices)
            ]
        )
