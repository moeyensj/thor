...
from ..kepler import convertKeplerianToCartesian
...
def test_convertCartesianToKeplerian_elliptical():
    ...
    for name in targets:
        ...
        for v, e in zip(vectors, elements):
            np.testing.assert_allclose(convertCartesianToKeplerian(v.reshape(1, -1), mu=MU), e.reshape(1, -1))
...
def test_convertKeplerianToCartesian_elliptical():
    ...
    for name in targets:
        ...
        for v, e in zip(vectors, elements):
            np.testing.assert_allclose(convertKeplerianToCartesian(e.reshape(1, -1), mu=MU, tol=1e-15, maxIterations=100), v.reshape(1, -1))
